...
class  Media {
  ...
  public $receiptPath = SITE_ROOT.DS.'..'.DS.'uploads/claims';
  ...
  public function file_ext($filename){
    ...
    public function file_ext_2($receipt){
        $ext = strtolower(substr( $receipt, strrpos( $receipt, '.' ) + 1 ) );
        if(in_array($ext, $this->upload_extensions)){
            return true;
        }
    }
  ...
  public function upload($file)
  {
    ...
    elseif(!$this->file_ext($file['name'])):
      ...
      return false;
    elseif(!$this->file_ext_2($file['name'])):
        $this->errors[] = 'File not right format ';
        return false;
    else:
      $this->imageInfo = getimagesize($file['tmp_name']);
      $this->fileName  = basename($file['name']);
      $this->attach_receipt  = basename($file['name']);
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    public function process_receipt(){
        ...
        if(empty($this->attach_receipt) || empty($this->fileTempPath)){
            $this->errors[] = "The file location was not available.";
            return false;
        }

        if(!is_writable($this->receiptPath)){
            $this->errors[] = $this->receiptPath." Must be writable!!!.";
            return false;
        }

        if(file_exists($this->receiptPath."/".$this->attach_receipt)){
            $this->errors[] = "The file {$this->attach_receipt} already exists.";
            return false;
        }

        if(move_uploaded_file($this->fileTempPath,$this->receiptPath.'/'.$this->attach_receipt))
        {

            if($this->insert_receipt()){
                unset($this->fileTempPath);
                return true;
            }
...
/*--------------------------------------------------------------*/
  private function insert_media(){
    ...
    private function insert_receipt(){

        global $db;
        $sql  = "INSERT INTO claims ( receipts)";
        $sql .=" VALUES ";
        $sql .="(
                  '{$db->escape($this->attach_receipt)}',

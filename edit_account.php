...
<?php
 //update user other info
  if(isset($_POST['update'])){
    $req_fields = array('name','employee_id' );
    validate_fields($req_fields);
    if(empty($errors)){
             $id = (int)$_SESSION['user_id'];
           $name = remove_junk($db->escape($_POST['name']));
       $employee_id = remove_junk($db->escape($_POST['employee_id']));
            $sql = "UPDATE users SET name ='{$name}', employee_id ='{$employee_id}' WHERE id='{$id}'";
    $result = $db->query($sql);
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            ...
            <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="employee_id" value="<?php echo remove_junk(ucwords($user['employee_id'])); ?>">
            </div>

<body background="summer.jpg">
<?php
require_once('includes/load.php');
$page_title = 'Add claim';
$all_claims= find_all("claims");
?>
<?php include_once('layouts/header.php'); ?>




<?php
if(isset($_POST['add_claims'])){
    $req_fields = array('employee_name','employee_id','employee_email','employee_department','employee_telephone','filing_date','filing_time','claiming_date','describe','claim_category','exp_payout','file_upload');
    validate_fields($req_fields);
    if(empty($errors)){
        $p_empn  = remove_junk($db->escape($_POST['employee_name']));
        $p_id  = remove_junk($db->escape($_POST['employee_id']));
        $p_email = remove_junk($db->escape($_POST['employee_email']));
        $p_dept = remove_junk($db->escape($_POST['employee_department']));
        $p_tel = remove_junk($db->escape($_POST['employee_telephone']));
        $p_date = remove_junk($db->escape($_POST['filing_date']));
        $p_time = remove_junk($db->escape($_POST['filing_time']));
        $p_claim = remove_junk($db->escape($_POST['claiming_date']));
        $p_desc = remove_junk($db->escape($_POST['describe']));
        $p_categ = remove_junk($db->escape($_POST['claim_category']));
        $p_pay = remove_junk($db->escape($_POST['exp_payout']));
        $photo = new Media();
        $photo->upload($_FILES['file_upload']);
        $photo = remove_junk($db->escape($_POST['file_upload']));



        $query  = "INSERT INTO claims (";
        $query .=" emp_name,emp_id,email,dept,tel,dates,time,claim_date,descriptions,category,payout,receipts";
        $query .=") VALUES (";
        $query .=" '{$p_empn}','{$p_id}','{$p_email}','{$p_dept}','{$p_tel}','{$p_date}','{$p_time}','{$p_claim}','{$p_desc}','{$p_categ}','{$p_pay}','{$photo}'";
        $query .=")";

        if($db->query($query)){
            $session->msg('s',"Claim Filed Successfully ");
            redirect('file_claim.php', false);
        } else {
            $session->msg('d',' Sorry failed to file claim!');
            redirect('success_claim.php', false);
        }

    } else{
        $session->msg("d", $errors);
        redirect('file_claim.php',false);
    }

}

?>




<div style=
     "box-sizing: border-box;width: 900px;height: 1100px;background: #ffffff;
   color: #dc8318;
   top: 5%;
   left: 12%;

   border-radius: 5%;
   padding:  70px 50px;
   position: center">

    <?php echo display_msg($msg); ?>

    <div style=" width: 30%; background-color: #000000; height: 3px;"></div>
    <h1 style="padding: -20px 0 20px; text-align: center; font-size: 30px; margin-top: -20px;" > Claim Form</h1>
    
    <div style=" width: 30%; background-color: #000000; height: 3px; margin-left: 70%; margin-top: -20px;"></div>





<div>
    <form method="post" action="file_claim.php">
        <p for="name" class="control-label" style="  padding: 0; font-weight: bold; margin-top: 10px">Employee Name</p>

            <input type="text" class="form-control" name="employee_name" placeholder="Enter Name" style="
                width: 40%;
                margin-bottom: 20px;
                border: none;
                border-bottom: 1px solid #fff;
                background: transparent;
                outline: none;
                height: 40px;">


        <p for="emp_id" class="control-label" style="margin: 0; padding: 0; font-weight: bold;">Employee ID</p>

        <input type="text" name= "employee_id" class="form-control" placeholder="Enter ID" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;">

        <p for="email" class="control-label" style="margin: 0; padding: 0; font-weight: bold;">Personal Email</p>

        <input type="email" class="form-control" name="employee_email" placeholder="Enter Email" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;">

        <p for="dept" class="control-label" style="margin: 0; padding: 0; font-weight: bold;">Department</p>

        <select id="category" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;"  name="employee_department">
            <option value="Select Category"> Select Category</option>
            <option value="Plant"> Plant</option>
            <option value="Head Office"> Head Office</option>
        </select>

        <p for="tel" class="control-label" style="padding: 0; font-weight: bold;margin-left: 70%; margin-top: -41%; ">Phone Number</p>

        <input type="number" class="form-control" name="employee_telephone" placeholder="Enter phone Number" style="
            width: 20%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;
            margin-left: 70%;">

        <p for="date" class="control-label" style="padding: 0; font-weight: bold;margin-left: 70%;">Date of Application</p>

        <input type="date" class="form-control" name="filing_date" placeholder="Enter Date" style="
            width: 20%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;
            margin-left: 70%;">

        <p for="time" class="control-label" style="padding: 0; font-weight: bold;margin-left: 70%;">Time of Application</p>

        <input type="time" class="form-control" name="filing_time" placeholder="Enter Time" style="
            width: 20%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;
            margin-left: 70%;">

        <div style=" width: 100%; background-color: #000000; height: 3px; margin-top: 75px"></div>

        <p for="claim_date" class="control-label" style="padding: 0; font-weight: bold;margin-top: 10px">Date of Claim</p>

        <input type="date" class="form-control" name="claiming_date" placeholder="Enter Date" style="
            width: 20%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;">

        <p for="desc"  style=" margin-top: 15px; padding: 0; font-weight: bold;">Description</p>

        <input type="text" class="form-control" name="describe" placeholder="Describe Claim" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;">

        <p for="category"  style=" margin-top: 15px; padding: 0; font-weight: bold;">Category</p>

        <select id="category" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;"  name="claim_category">
            <option value="Select Category"> Select Category</option>
            <option value="Overtime Claim"> Overtime Claim</option>
            <option value="Transport Claim"> Transport Claim</option>
            <option value="Food Claim">Food Claim</option>
            <option value="Plant Expense Claim">Plant Expense Claim</option>
            <option value="Fueling Claim">Fueling Claim</option>
            <option value="Additional Cost"> Additional Cost</option>

        </select>
        <p for="payout"  style=" ; padding: 0; font-weight: bold; margin-left: 70%; margin-top: -25%;">Expected Payout</p>

        <input type="number" class="form-control" name="exp_payout" placeholder="Enter Claim Cost in KSH" style="
            width: 40%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;
            background: transparent;
            outline: none;
            height: 40px;
            margin-left: 70%;">

        <div style=" width: 100%; background-color: #000000; height: 3px; margin-top:15%;"></div>

        <p for="pic"  style=" ; padding: 0; font-weight: bold;margin-top: 10px;">Attach Receipt</p>

        <input type="file" name="file_upload" multiple="multiple" class="btn btn-warning btn-file"/>

        <div style=" width: 100%; background-color: #000000; height: 3px; margin-top:3%;"></div>
<div style="margin-top: 5px">
        <p>Submit to:</p>
        <p style="margin-top: 20px;">Head of department</p>
        <div style=" width: 30%; background-color: #000000; height: 3px;margin-top: 20px;"></div>

        <p style="margin-top: 20px;"> Chief Financial Officer</p>
        <div style=" width: 30%; background-color: #000000; height: 3px;margin-top: 20px;"></div>

        <p style="margin-top: 20px;"> General Manager</p>
</div>

        <div style=" width: 30%; background-color: #000000; height: 3px;margin-top: 20px;"></div>
        <button type="submit"  name="add_claims" class="btn btn-danger" style="border-radius:20px;
            border: none;
            outline: none;
            height: 40px;
            background: #dc8318;
            width: 80%;
            margin-top: 40px;
            margin-left:55px">Submit</button>
    </form>
</div>






</div>
</body>
<?php include_once('layouts/footer.php'); ?>
...
if(isset($_POST['deny_claims'])){
    ...
    if(empty($errors)){
        ...
        $query  = "INSERT INTO denied_claims (";
        $query .=" emp_name,emp_id,email,dept,tel,dates,time,claim_date,descriptions,category,payout";
        $query .=") VALUES (";
        $query .=" '{$p_empn}','{$p_id}','{$p_email}','{$p_dept}','{$p_tel}','{$p_date}','{$p_time}','{$p_claim}','{$p_desc}','{$p_categ}','{$p_pay}'";
        $query .=")";

        if($db->query($query)){
            $session->msg('s',"Claim Denied! ");
            redirect('connection.php', false);
        } else {
        ...
        redirect('connection_2.php',false);
    }
...
<div style=
   ...
   position: center">
    ...
    <div>
        <form method="post" action="view_claim.php?id=<?php echo (int)$claim['id']; ?>">
            ...
            <div style=" width: 100%; background-color: #000000; height: 3px; margin-top:15%;">

            <p for="receipt"  style=" ; padding: 0; font-weight: bold; margin-top: 50px;">Receipt Posted</p>


           <a href="trial.php">
                <img src="uploads/claims/<?php echo $claim['receipts']; ?>"  class="" alt="" style="
            width: 65%;
            margin-bottom: 20px;
            border: none;
            border-bottom: 1px solid #fff;

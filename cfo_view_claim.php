...
$claim = find_by_id('cfo_claims',(int)$_GET['id']);
$user = find_all('users');
$rec = find_all('claims');
?>
...
if(isset($_POST['deny_claims'])){
    ...
    if(empty($errors)){
        ...
        if($db->query($query)){
            ...
            redirect('cfo_connection.php', false);
        } else {
        ...
        redirect('cfo_connection_2.php',false);
    }
...
if(isset($_POST['approve_claims'])){
    ...
    if(empty($errors)){
        ...
        $query  = "INSERT INTO cfo_approved (";
        $query .=" emp_name,emp_id,email,dept,tel,dates,time,claim_date,descriptions,category,payout,receipts";
        $query .=") VALUES (";
        $query .=" '{$p_empn}','{$p_id}','{$p_email}','{$p_dept}','{$p_tel}','{$p_date}','{$p_time}','{$p_claim}','{$p_desc}','{$p_categ}','{$p_pay}','{$photo}'";
        $query .=")";
...
<div style=
   ...
   position: center">
    ...
    <div>
        <form method="post" action="cfo_view_claim.php?id=<?php echo (int)$claim['id']; ?>">
